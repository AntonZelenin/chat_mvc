    ...
    protected function _tick()
    {
        ...
        foreach ($this->heldMessages as $key => $hm) {
            ...
            foreach ($this->users as $currentUser) {
                if ($hm['user']->socket == $currentUser->get_socket()) {
                    $found = true;

                    if ($currentUser->is_handshake()) {
    ...
    */
    ...
    protected function disconnect($socket, $triggerClosed = true, $sockErrNo = null)
    {
        ...
        if ($disconnectedUser !== null) {
            ...
            if ($triggerClosed) {
                $this->stdout("Client disconnected. ".$disconnectedUser->get_socket());
                $this->closed($disconnectedUser);
                socket_close($disconnectedUser->get_socket());
            } else {
                $message = $this->frame('', $disconnectedUser, 'close');
                @socket_write($disconnectedUser->get_socket(), $message, strlen($message));
            }
        }
    }
    ...
    protected function doHandshake($user, $buffer)
    {
        ...
        if (isset($handshakeResponse)) {
            socket_write($user->get_socket(), $handshakeResponse, strlen($handshakeResponse));
            $this->disconnect($user->get_socket());
            return;
        }

        $user->set_headers($headers);
        $user->set_handshake($buffer);
    ...
    protected function getUserByDatabaseId($id)
    {
        foreach ($this->users as $user) {
            if ($user->get_database_id() == $id) {
                return $user;
            }
        }
    ...
    protected function deframe($message, $user)
    {
        ...
        */
        ...
        if ($pongReply) {
            ...
            socket_write($user->get_socket(), $reply, strlen($reply));
            return false;
        }
